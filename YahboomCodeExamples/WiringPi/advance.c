...
void run(int time)
{
  ...
  softPwmWrite(Left_motor_pwm, 100);    //pwm:0-255. The left and right wheels are slightly different

  //Right motor advance 
  digitalWrite(Right_motor_go, HIGH);   
  digitalWrite(Right_motor_back, LOW);  
  //This code is for setting the PWM of the specified pin
  softPwmWrite(Right_motor_pwm, 100);   //pwm:0-255. The left and right wheels are slightly different
